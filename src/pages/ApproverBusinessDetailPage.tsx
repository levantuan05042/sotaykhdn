...
export const ApproverBusinessDetailPage: React.FC = () => {
  ...
  const fetchDetail = async () => {
    ...
    try {
      ...
      setDetail({
        ...
        createdBy: busData.createdByFullName || busData.createdBy,
        approvedBy: busData.approvedByFullName || busData.approvedBy,
  ...
  const handleSaveReview = async (statusVal: string, commentVal: string) => {
    ...
    try {
      ...
      toast.success(statusVal === 'ACTIVE' ? 'Phê duyệt danh mục sản phẩm cấp 2 thành công!' : 'Đã phản hồi ý kiến đánh giá!');
      fetchDetail();
    } catch (error: any) {
