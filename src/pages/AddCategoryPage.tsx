...
const AddCategoryPage: React.FC = () => {
  ...
  // Trạng thái Dropdown Nhóm sản phẩm
  ...
  // Gọi API lấy danh sách nhóm sản phẩm đang hoạt động
  useEffect(() => {
  ...
  // Xử lý tự động đóng bộ chọn khi bấm trượt ra ngoài
  useEffect(() => {
  ...
  const onSaveDraftClick = () => {
    const nameErr = getNameError(formData.name, 'Tên danh mục sản phẩm cấp 1');
    if (nameErr) {
  ...
  const onSubmitClick = () => {
    ...
    if (!formData.groupId) {
      toast.error("Vui lòng chọn nhóm sản phẩm thuộc về", { position: 'top-center' });
      setIsOpen(true);
      return;
    }
  ...
  const handleCreateCategory = async (status: 'DRAFT' | 'PENDING_APPROVAL') => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Tên danh mục sản phẩm cấp 1 đã tồn tại một bản trùng';
  ...
  return (
    <div className="pageWrapper">
        ...
        <div className="header">
          <div className="headerLeft">
            <button className="btnBack" onClick={handleGoBack}>
              ...
              <span className="breadcrumbText">Quản lý danh mục sản phẩm cấp 1</span>
            </button>
            ...
            <div className="breadcrumb">
              ...
              <span className="breadcrumbActive">Tạo mới danh mục sản phẩm cấp 1</span>
            </div>
          </div>
            ...
            <div className="formCard">
              ...
              <div className="formGroup">
                ...
                <input 
                  type="text" 
                  name="name" 
                  className={`input ${getNameError(formData.name, 'Tên danh mục sản phẩm cấp 1') ? 'input-invalid' : ''}`}
                  placeholder="Nhập tên danh mục sản phẩm cấp 1..."
                  value={formData.name} 
          ...
          <div className="rightCol" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
             ...
             <div className="commentCard emptyComment">
                ...
                <div className="emptyStateText">
                  Bình luận sẽ hiển thị sau khi danh mục sản phẩm cấp 1 được khởi tạo.
                </div>
             </div>
          </div>
        </div>
      ...
      <ActionConfirmModal
        ...
        desc={confirmAction === 'DRAFT' ? 'Bạn có chắc chắn muốn lưu bản nháp danh mục sản phẩm cấp 1 không?' : 'Bạn có chắc chắn muốn gửi phê duyệt danh mục sản phẩm cấp 1 không?'}
