...
const ProductGroupPage: React.FC = () => {
  ...
  // Gọi API lấy danh sách Nhóm sản phẩm để cho vào bộ lọc Dropdown
  useEffect(() => {
    const fetchGroupOptions = async () => {
      try {
        ...
        const mappedGroups = (response.data || [])
          .filter((item: any) => 
            item.status === 'ACTIVE' && item.active === true
          )
          .map((item: any) => ({
            value: item.id,
            label: item.name
  ...
  return (
    <div className="product-group-container">
      ...
      <div className="content-wrapper">
        <h2 className="page-title">Quản lý danh sách sản phẩm</h2>
