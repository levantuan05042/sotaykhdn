...
const ProductBusinessPage: React.FC = () => {
  ...
  const cached = getCachedPageState('product-business');
  ...
  // Cascade hide state
  const [cascadeTarget, setCascadeTarget] = useState<ProductBusinessItem | null>(null);
  ...
  useEffect(() => {
    setCachedPageState('product-business', {
      searchTerm,
      selectedStatuses,
      selectedGroups,
      selectedCategories,
  ...
  }, [searchTerm, selectedStatuses, selectedGroups, selectedCategories, selectedActives, selectedCreators, selectedApprovers, currentPage]);
  ...
  const fetchData = async (isBackground = false) => {
    ...
    try {
      ...
      if (!isBackground) {
        restorePageScroll('product-business');
      }
  ...
  const categoryFilterOptions = useMemo(() => {
    ...
    data.forEach(item => {
      if (item.categoryId && item.categoryName) {
        ...
        map.set(item.categoryName, item.categoryName);
      }
    });
    return Array.from(map.entries()).map(([value, label]) => ({ value, label }));
  ...
  const getFilteredData = () => {
    return data.filter(item => {
      ...
      if (selectedCategories.length > 0 && !selectedCategories.some(c => c === item.categoryId || c === item.categoryName)) {
        return false;
      }
  ...
  const columns: Column<ProductBusinessItem>[] = [
    ...
    },
    {
      ...
      render: (row) => (
        <CellWithTooltip text={row.groupName} />
      ),
    },
    {
      key: 'categoryName',
    ...
    <div className="product-group-container">
      ...
      <div className="filter-section" ref={filterSectionRef}>
        <div className="dropdown-group-container">
          ...
          <div className="selected-filters-row">
            ...
            {(selectedGroups.length > 0 || selectedCategories.length > 0 || selectedStatuses.length > 0 || selectedActives.length > 0 || selectedCreators.length > 0 || selectedApprovers.length > 0) && (
              <button
                type="button"
                onClick={() => {
                  setSelectedGroups([]);
      ...
      <div className="table-placeholder" style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', minHeight: 0 }}>
        <DataTable
          ...
          onRowClick={(row) => {
            savePageScroll('product-business');
            navigate(`/business-management/${row.id}`);
          }}
          loading={loading}
          page={currentPage}
