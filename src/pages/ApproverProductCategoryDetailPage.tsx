...
export const ApproverProductCategoryDetailPage: React.FC = () => {
  ...
  const fetchDetail = async () => {
    ...
    try {
      ...
      setDetail({
        ...
        createdBy: catData.createdByFullName || catData.createdBy,
        approvedBy: catData.approvedByFullName || catData.approvedBy,
  ...
  const handleSaveReview = async (statusVal: string, commentVal: string) => {
    ...
    try {
      ...
      toast.success(statusVal === 'ACTIVE' ? 'Phê duyệt danh mục sản phẩm cấp 1 thành công!' : 'Đã phản hồi ý kiến đánh giá!');
      fetchDetail();
    } catch (error: any) {
