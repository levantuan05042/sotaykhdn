...
export const ActionConfirmModal: React.FC<ActionConfirmModalProps> = ({
  ...
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && isOpen && !loading && !confirming) {
        onClose();
      }
    };
  ...
  const ignoreGhostClick = () => Date.now() - openedAtRef.current < 400;

  const handleOverlayClick = (e: React.MouseEvent) => {
    if (e.target !== e.currentTarget) return;
    if (isBusy || ignoreGhostClick()) return;
    onClose();
  };
      ...
      >
        ...
        <div className="confirm-toast-actions" style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
          <button
            ...
            onMouseEnter={(e) => {
              if (!isBusy) e.currentTarget.style.backgroundColor = '#F3F4F6';
            }}
            onMouseLeave={(e) => {
